...
export async function getVideoDeadline() {
  ...
  if (data?.last_posted_at) {
    lastPostedStr = data.last_posted_at;
    if (typeof window !== 'undefined') localStorage.setItem('video_timer_fallback', lastPostedStr);
  } else {
    lastPostedStr = typeof window !== 'undefined' ? localStorage.getItem('video_timer_fallback') : null;
    if (!lastPostedStr) {
      lastPostedStr = new Date().toISOString();
      if (typeof window !== 'undefined') localStorage.setItem('video_timer_fallback', lastPostedStr);
    }
  }

  const lastPosted = new Date(lastPostedStr);
  const deadline = new Date(lastPosted.getTime() + 48 * 60 * 60 * 1000);
  const now = new Date();
  
  const diff = deadline - now;
  if (diff <= 0) {
    return { hoursLeft: 0, minutesLeft: 0, secondsLeft: 0, isOverdue: true, targetTime: deadline.getTime() };
  }

  const hoursLeft = Math.floor(diff / (1000 * 60 * 60));
  const minutesLeft = Math.floor((diff % (1000 * 60 * 60)) / (1000 * 60));
  const secondsLeft = Math.floor((diff % (1000 * 60)) / 1000);
  return { hoursLeft, minutesLeft, secondsLeft, isOverdue: false, targetTime: deadline.getTime() };
}

export async function resetVideoTimer() {
  const nowStr = new Date().toISOString();
  if (typeof window !== 'undefined') localStorage.setItem('video_timer_fallback', nowStr);
  return await supabase
    .from('video_timer')
    .upsert([{ id: 1, last_posted_at: nowStr, updated_at: nowStr }]);
}
